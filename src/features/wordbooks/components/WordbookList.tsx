...
export default function WordbookList({
  ...
  // サーバーアクション
  const [createErr, createAction, createPending] = useActionState(
    createWordbook,
    undefined,
  );
  const [updateErr, updateAction, updatePending] = useActionState(
    updateWordbook,
    undefined,
  );
  const [deleteErr, deleteAction, deletePending] = useActionState(
    deleteWordbook,
    undefined,
  );
    ...
    <Box sx={{ p: 3 }}>
      ...
      {(createErr || updateErr || deleteErr) && (
        <Alert severity="error" sx={{ mb: 2 }}>
          {String(createErr || updateErr || deleteErr)}
        </Alert>
      )}
      ...
      >
        ...
        <form action={updateAction} onSubmit={() => setEditId(null)}>
          ...
          <DialogActions>
            ...
            <Button type="submit" variant="contained" disabled={updatePending}>
              保存
            </Button>
          </DialogActions>
        </form>
